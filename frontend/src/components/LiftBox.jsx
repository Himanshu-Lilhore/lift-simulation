...
export default function LiftBox({ index, state, setState, floors, inMotion, isOpen, setIsOpen, setInMotion, animationEndsAt, setAnimationEndsAt, taskCount, setTaskCount }) {
    const doorMotion = 2500, doorPause = 500, timeBtwFloors = 2000  // time in ms
    ...
    useEffect(() => {
        if (state[index] && !inMotion[index] && !state[index].fulfillment && !isOpen[index]) {
            setIsOpen(prev => {
                const neww = [...prev]
                neww[index] = true
                return neww
    ...
    return (
        <div
            className={`liftbox absolute m-3 border-y-[1.2rem] border-x-4 border-black w-20 h-32 rounded-sm overflow-hidden`}
            style={{
                left: '0%',
        ...
        >
            <div id='left-panel' className={`${!(isOpen[index]) && 'active'}`}></div>
            <div id='right-panel' className={`${!(isOpen[index]) && 'active'}`}></div>
        </div>
    );
}
